...
export default function InterviewPrep() {
  ...
  const QCard = ({ q, idx, dk }) => {
    ...
    return (
      <div style={{ background:"white", border:`1.5px solid ${open ? cfg.color+"55" : "rgba(0,0,0,.08)"}`, borderRadius:13, marginBottom:10, overflow:"hidden", transition:"border .15s", boxShadow:open?`0 4px 20px ${cfg.color}18`:"0 1px 3px rgba(0,0,0,.05)" }}>
        ...
        <div onClick={() => toggle(key)} style={{ padding:"14px 16px", cursor:"pointer", display:"flex", gap:12, alignItems:"flex-start" }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:14, fontWeight:600, color:"#0f172a", lineHeight:1.6, marginBottom:7 }}>{q.q}</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
              {q.difficulty && <span style={{ padding:"2px 9px", background:DIFF_BG[q.difficulty]||"#f1f5f9", borderRadius:5, fontSize:11, fontWeight:700, color:DIFF_COLOR[q.difficulty]||"#475569" }}>{q.difficulty}</span>}
          ...
          <div style={{ display:"flex", gap:5, flexShrink:0, alignItems:"center" }}>
            ...
            <div style={{ width:26, height:26, borderRadius:"50%", background:open?cfg.color:"rgba(0,0,0,.07)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color:open?"white":"#64748b", transition:"all .18s" }}>
              {open?"▲":"▼"}
            </div>
          </div>
        </div>
  ...
  if (phase === "landing") return (
    <div className="fade-up" style={{ maxWidth:820, margin:"0 auto" }}>
      <div style={{ textAlign:"center", marginBottom:44 }}>
        ...
        <h1 style={{ fontSize:34, fontWeight:800, color:"#0f172a", letterSpacing:"-.8px", marginBottom:10 }}>Interview Question Predictor</h1>
        <p style={{ fontSize:15, color:"#475569", maxWidth:520, margin:"0 auto", lineHeight:1.75 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:36 }}>
        {Object.entries(DIV_CONFIG).map(([k,cfg]) => (
          <div key={k} style={{ background:"white", border:`2px solid ${cfg.color}22`, borderRadius:16, padding:"22px 18px", textAlign:"center" }}>
            <div style={{ fontSize:32, marginBottom:10 }}>{cfg.icon}</div>
            <div style={{ fontSize:15, fontWeight:800, color:"#0f172a", marginBottom:6 }}>{cfg.label}</div>
            <div style={{ fontSize:12, color:"#64748b", lineHeight:1.6 }}>{cfg.desc}</div>
          </div>
        ))}
      </div>

      {/* Role input */}
      <div style={{ background:"white", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:18, padding:"28px", boxShadow:"0 4px 24px rgba(0,0,0,.06)", marginBottom:24 }}>
        <div style={{ fontSize:15, fontWeight:700, color:"#0f172a", marginBottom:6 }}>Enter your target job role</div>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <input value={inputRole} onChange={e => setInputRole(e.target.value)}
            ...
            style={{ flex:1, padding:"13px 16px", background:"#f8fafc", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:10, fontSize:14, color:"#0f172a", outline:"none" }}
      ...
      {/* Popular roles */}
      <div style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"1px", marginBottom:12 }}>Quick select</div>
      <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
        {POPULAR_ROLES.map(r => (
          <button key={r} onClick={() => confirmRole(r)}
            style={{ padding:"8px 16px", background:"white", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:22, fontSize:12.5, fontWeight:500, color:"#374151", cursor:"pointer", transition:"all .15s" }}
            ...
            onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(0,0,0,.1)";e.currentTarget.style.color="#374151";e.currentTarget.style.background="white";}}>
            {r}
          </button>
        ))}
      </div>
  ...
  if (phase === "divisions") return (
    <div className="fade-up" style={{ maxWidth:700, margin:"0 auto" }}>
      <button onClick={() => setPhase("landing")}
        style={{ background:"none", border:"none", color:"#64748b", fontSize:13, cursor:"pointer", marginBottom:24, display:"flex", alignItems:"center", gap:5 }}>
        ← Change Role
      </button>

      <div style={{ textAlign:"center", marginBottom:40 }}>
        <div style={{ fontSize:13, color:"#64748b", marginBottom:6 }}>Generating questions for</div>
        <h2 style={{ fontSize:30, fontWeight:800, color:"#0f172a", letterSpacing:"-.5px", marginBottom:0 }}>{role}</h2>
      </div>

      <div style={{ fontSize:15, fontWeight:700, color:"#0f172a", textAlign:"center", marginBottom:24 }}>
        ...
        {Object.entries(DIV_CONFIG).map(([k, cfg]) => {
          ...
          return (
            <button key={k} onClick={() => loadDivision(k)} disabled={isLoading}
              style={{ display:"flex", alignItems:"center", gap:18, padding:"22px 24px", background:"white", border:`2px solid ${isDone ? cfg.color : hasErr ? "#fca5a5" : "rgba(0,0,0,.1)"}`, borderRadius:16, cursor:isLoading?"wait":"pointer", textAlign:"left", transition:"all .2s", boxShadow: isDone ? `0 4px 16px ${cfg.color}18` : "0 2px 8px rgba(0,0,0,.04)" }}
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:17, fontWeight:800, color:"#0f172a", marginBottom:4 }}>{cfg.label}</div>
              ...
              <div style={{ fontSize:13, fontWeight:600, color: isDone?cfg.color:"#94a3b8", flexShrink:0 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22, flexWrap:"wrap", gap:12 }}>
        <div>
          <button onClick={() => setPhase("divisions")}
            style={{ background:"none", border:"none", color:"#64748b", fontSize:13, cursor:"pointer", marginBottom:6, display:"flex", alignItems:"center", gap:5 }}>
            ← Back to Sections
          </button>
          <h2 style={{ fontSize:26, fontWeight:800, color:"#0f172a", letterSpacing:"-.4px", marginBottom:4 }}>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={() => { setLoaded(p => ({ ...p, [activeDivision]: undefined })); loadDivision(activeDivision); }}
            style={{ padding:"9px 16px", background:"white", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:9, color:"#374151", fontSize:13, fontWeight:600, cursor:"pointer" }}>
            🔄 Refresh
          </button>
        ...
        {Object.entries(DIV_CONFIG).map(([k, c]) => {
          ...
          return (
            <button key={k} onClick={() => { if (loaded[k]) { setActiveDiv(k); } else { setPhase("divisions"); loadDivision(k); } }}
              style={{ padding:"8px 16px", border:`2px solid ${active?c.color:"rgba(0,0,0,.1)"}`, borderRadius:22, fontSize:13, fontWeight:active?700:500, background:active?c.color+"15":"white", color:active?c.color:"#475569", cursor:"pointer", transition:"all .15s", display:"flex", alignItems:"center", gap:6 }}>
              {c.icon} {c.label}
              {count > 0 && <span style={{ background:active?c.color+"25":"rgba(0,0,0,.06)", padding:"1px 7px", borderRadius:10, fontSize:11, color:active?c.color:"#64748b", fontWeight:700 }}>{count}</span>}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:20, flexWrap:"wrap", alignItems:"center" }}>
        <div style={{ position:"relative", flex:1, minWidth:220 }}>
          ...
          <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Search questions and answers…"
            style={{ width:"100%", padding:"9px 12px 9px 34px", background:"white", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:10, fontSize:13, color:"#0f172a", outline:"none" }}
        ...
        <div style={{ display:"flex", gap:6 }}>
          {["All","Easy","Medium","Hard"].map(d => (
            <button key={d} onClick={() => setDiff(d)}
              style={{ padding:"7px 13px", border:`1.5px solid ${diffFilter===d?(DIFF_COLOR[d]||cfg.color):"rgba(0,0,0,.1)"}`, borderRadius:8, fontSize:12, fontWeight:diffFilter===d?700:500, background:diffFilter===d?(DIFF_BG[d]||cfg.color+"14"):"white", color:diffFilter===d?(DIFF_COLOR[d]||cfg.color):"#475569", cursor:"pointer" }}>
              {d}
            </button>
          ))}
          {bmCount > 0 && (
            <button onClick={() => setDiff("bookmarks")}
              style={{ padding:"7px 13px", border:`1.5px solid ${diffFilter==="bookmarks"?"#d97706":"rgba(0,0,0,.1)"}`, borderRadius:8, fontSize:12, fontWeight:diffFilter==="bookmarks"?700:500, background:diffFilter==="bookmarks"?"rgba(217,119,6,.08)":"white", color:diffFilter==="bookmarks"?"#d97706":"#475569", cursor:"pointer" }}>
              ⭐ Saved ({bmCount})
            </button>
          )}
        </div>
      ...
      {errors[activeDivision] && (loaded[activeDivision]||[]).length === 0 && (
        <div style={{ padding:"16px 18px", background:"rgba(220,38,38,.05)", border:"1.5px solid rgba(220,38,38,.15)", borderRadius:12, marginBottom:16, display:"flex", gap:12, alignItems:"flex-start" }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:"#64748b" }}>{errors[activeDivision]}</div>
          </div>
        </div>
      )}
