...
import { API, EMPTY_RESUME, Spinner, Label, Tag,
         AddRow } from "./constants";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
      <svg width={110} height={110} viewBox="0 0 110 110">
        ...
        />
        <text x={55} y={52} textAnchor="middle" fill={color} fontSize={22} fontWeight={800} fontFamily="Bricolage Grotesque, system-ui, sans-serif">{score}</text>
        <text x={55} y={67} textAnchor="middle" fill="#8888a8" fontSize={10} fontFamily="Inter Tight,sans-serif">ATS Score</text>
      </svg>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      {top.map((s, i) => {
        ...
        return (
          <div key={i} style={{ padding: "9px 11px", background: "#16161f", border: `1px solid ${impactColor}22`, borderRadius: 9, fontSize: 11 }}>
            ...
            <div style={{ color: "#444460", lineHeight: 1.5 }}>{s.fix}</div>
          </div>
        );
      })}
    </div>
...
function ExpEntry({ exp, onChange, onRemove, idx }) {
  ...
  return (
    <div style={{ background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 12, padding: 18, marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--teal)", fontFamily: "Bricolage Grotesque, system-ui, sans-serif" }}>Experience #{idx + 1}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        <div>
          <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Job Title *</div>
        ...
        <div>
          <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Company *</div>
        ...
        <div>
          <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Location</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
          <div>
            <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Start</div>
          ...
          <div>
            <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>End</div>
            <input className="dark-input" style={{ width: "100%" }} value={exp.end || ""} onChange={e => onChange({ ...exp, end: e.target.value })} placeholder="Present" />
          </div>
        </div>
      </div>
      <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 6 }}>Key Achievements / Responsibilities <span style={{ color: "#8888a8" }}>(use action verbs + numbers)</span></div>
      {[0, 1, 2, 3].map(bi => (
  ...
  return (
    <div className="fade-up" style={{ maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={{ textAlign:"center", marginBottom:48 }}>
        ...
        <h1 style={{ fontSize:36, fontWeight:800, color:"#f1f1f5", letterSpacing:"-.8px", marginBottom:10 }}>Resume Maker</h1>
        <p style={{ fontSize:16, color:"#8888a8", maxWidth:480, margin:"0 auto", lineHeight:1.7 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:40 }} className="two-col">
        ...
        <button onClick={() => setMainMode("ats_score")}
          style={{ background:"#1c1c28", border:"2px solid rgba(0,0,0,.08)", borderRadius:20, padding:"36px 32px", textAlign:"left", cursor:"pointer", transition:"all .2s", display:"flex", flexDirection:"column", gap:0 }}
          ...
          <div style={{ fontSize:22, fontWeight:800, color:"#f1f1f5", marginBottom:8, letterSpacing:"-.3px" }}>Check ATS Score</div>
          <div style={{ fontSize:14, color:"#8888a8", lineHeight:1.7, marginBottom:20 }}>
        ...
        <button onClick={() => setMainMode("build")}
          style={{ background:"#1c1c28", border:"2px solid rgba(0,0,0,.08)", borderRadius:20, padding:"36px 32px", textAlign:"left", cursor:"pointer", transition:"all .2s", display:"flex", flexDirection:"column" }}
          ...
          <div style={{ fontSize:22, fontWeight:800, color:"#f1f1f5", marginBottom:8, letterSpacing:"-.3px" }}>Build from Scratch</div>
          <div style={{ fontSize:14, color:"#8888a8", lineHeight:1.7, marginBottom:20 }}>
      ...
      {/* Import option */}
      <div style={{ background:"#16161f", border:"1.5px dashed rgba(0,0,0,.12)", borderRadius:14, padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          <span style={{ fontSize:28 }}>📂</span>
          <div>
            <div style={{ fontSize:14, fontWeight:700, color:"#f1f1f5" }}>Already have a resume PDF?</div>
            <div style={{ fontSize:13, color:"#8888a8", marginTop:2 }}>Import it to pre-fill the builder and get an instant ATS score.</div>
          </div>
        </div>
...
function ATSScoreChecker({ onBack }) {
  ...
  return (
    <div className="fade-up" style={{ maxWidth:720, margin:"0 auto" }}>
      <button onClick={onBack} style={{ background:"none", border:"none", color:"#8888a8", fontSize:13, cursor:"pointer", marginBottom:24, display:"flex", alignItems:"center", gap:6 }}>← Back to Resume Maker</button>
      <h2 style={{ fontSize:28, fontWeight:800, color:"#f1f1f5", marginBottom:6, letterSpacing:"-.4px" }}>ATS Score Checker</h2>
      <p style={{ color:"#8888a8", fontSize:14, marginBottom:32, lineHeight:1.7 }}>Upload your resume PDF to get an instant ATS score, keyword analysis, and ranked improvement suggestions.</p>

      {!result ? (
        <>
          <label style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16, padding:"56px 32px", border:`2px dashed ${file ? "var(--teal)" : "rgba(245,158,11,0.2)"}`, borderRadius:18, cursor:"pointer", background:file ? "rgba(13,148,136,.04)" : "rgba(255,255,255,0.02)", transition:"all .2s" }}
            onMouseEnter={e => { if (!file) { e.currentTarget.style.borderColor="var(--teal)"; e.currentTarget.style.background="rgba(13,148,136,.03)"; }}}
            onMouseLeave={e => { if (!file) { e.currentTarget.style.borderColor="rgba(245,158,11,0.2)"; e.currentTarget.style.background="rgba(255,255,255,0.02)"; }}}>
            ...
            <div style={{ textAlign:"center" }}>
              ...
              <div style={{ fontSize:13, color:"#444460" }}>PDF files only · Text-based (not scanned)</div>
            </div>
          </label>
        ...
        <div className="fade-in">
          {/* Score hero */}
          <div style={{ background:"#1c1c28", border:"2px solid rgba(0,0,0,.08)", borderRadius:20, padding:"32px", textAlign:"center", marginBottom:20, boxShadow:"0 4px 24px rgba(0,0,0,.06)" }}>
            ...
            <div style={{ fontSize:14, color:"#8888a8" }}>Your resume scored <strong style={{ color:scoreColor }}>{result.score}/100</strong> on ATS systems</div>

            {result.word_count && <div style={{ marginTop:10, fontSize:12, color:"#444460" }}>Word count: {result.word_count} · Extraction: {result.extraction_method}</div>}
          </div>

          {/* Score breakdown */}
          <div style={{ background:"#1c1c28", border:"1.5px solid rgba(0,0,0,.08)", borderRadius:16, padding:"20px 24px", marginBottom:16, boxShadow:"0 2px 8px rgba(0,0,0,.04)" }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#444460", letterSpacing:"1px", textTransform:"uppercase", marginBottom:16 }}>Score Breakdown</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }} className="two-col">
            {/* Keywords */}
            <div style={{ background:"#1c1c28", border:"1.5px solid rgba(0,0,0,.08)", borderRadius:16, padding:"20px 24px" }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#444460", letterSpacing:"1px", textTransform:"uppercase", marginBottom:14 }}>Keywords Found</div>
              ...
              <div style={{ fontSize:11, fontWeight:700, color:"#444460", letterSpacing:"1px", textTransform:"uppercase", marginBottom:10 }}>Missing Keywords</div>
            ...
            {/* Top suggestions */}
            <div style={{ background:"#1c1c28", border:"1.5px solid rgba(0,0,0,.08)", borderRadius:16, padding:"20px 24px" }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#444460", letterSpacing:"1px", textTransform:"uppercase", marginBottom:14 }}>Top Fixes (by impact)</div>
              {(result.suggestions || []).slice(0,5).map((s,i) => {
                const ic = s.impact >= 5 ? "#dc2626" : s.impact >= 3 ? "#d97706" : "var(--teal)";
                return (
                  <div key={i} style={{ padding:"10px 12px", background:"#16161f", border:`1px solid ${ic}22`, borderRadius:9, marginBottom:8 }}>
                    ...
                    <div style={{ fontSize:12, color:"#8888a8", lineHeight:1.5 }}>{s.fix}</div>
                  </div>
                );
              })}
            </div>
...
function TemplateGallery({ onBack, onTemplate, onScratch }) {
  ...
  const MiniResume = ({ tpl }) => {
    ...
    return (
      <div style={{ height:160, background:"#1c1c28", overflow:"hidden", fontFamily:"Arial,sans-serif", fontSize:4.5, lineHeight:1.4, padding:"9px 10px", position:"relative" }}>
        <div style={{ borderBottom:`1.5px solid ${c}`, paddingBottom:4, marginBottom:4 }}>
          <div style={{ fontWeight:700, fontSize:6.5, color:"#f1f1f5" }}>Your Name</div>
          <div style={{ color:"#8888a8", fontSize:4, marginTop:1 }}>{r.personal.email || "you@email.com"} · {r.personal.phone || "+91 98765 43210"}</div>
        </div>
        ...
        {r.experience?.[0] && <div style={{ marginBottom:4 }}>
          ...
          <div style={{ fontWeight:700, color:"#f1f1f5" }}>{r.experience[0].title}</div>
          <div style={{ color:"#8888a8" }}>{r.experience[0].company}</div>
        </div>}
        <div>
    ...
    <div className="fade-up">
      ...
      {preview && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20, backdropFilter:"blur(4px)" }}
          ...
          <div style={{ background:"#1c1c28", borderRadius:20, overflow:"hidden", maxWidth:640, width:"100%", maxHeight:"90vh", display:"flex", flexDirection:"column" }}
            ...
            <div style={{ padding:"16px 20px", borderBottom:"1px solid rgba(0,0,0,.08)", display:"flex", justifyContent:"space-between", alignItems:"center", background:`${preview.color}08`, flexShrink:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  <div style={{ fontWeight:700, fontSize:16, color:"#f1f1f5" }}>{preview.name}</div>
                  <div style={{ fontSize:12, color:"#8888a8" }}>{preview.category} · {preview.tags.join(", ")}</div>
                </div>
              </div>
              <button onClick={() => setPreview(null)} style={{ background:"rgba(255,255,255,0.06)", border:"none", borderRadius:8, padding:"6px 12px", cursor:"pointer", fontSize:13, color:"#8888a8" }}>✕ Close</button>
            </div>
            <div style={{ overflowY:"auto", flex:1, padding:20, background:"#16161f" }}>
              <div style={{ background:"#1c1c28", borderRadius:10, overflow:"hidden", boxShadow:"0 2px 12px rgba(0,0,0,.08)" }}>
                <ResumePreview
...
/>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28, flexWrap:"wrap", gap:12 }}>
        <div>
          <button onClick={onBack} style={{ background:"none", border:"none", color:"#8888a8", fontSize:13, cursor:"pointer", marginBottom:8, display:"flex", alignItems:"center", gap:6 }}>← Back</button>
          <h2 style={{ fontSize:28, fontWeight:800, color:"#f1f1f5", letterSpacing:"-.4px" }}>Choose a Template</h2>
          <p style={{ fontSize:14, color:"#8888a8", marginTop:4 }}>150 professional templates across 15 categories</p>
        </div>
        <button onClick={onScratch} style={{ padding:"10px 20px", background:"#1c1c28", border:"1.5px solid rgba(0,0,0,.12)", borderRadius:10, fontSize:13, fontWeight:600, color:"#374151", cursor:"pointer" }}>
          Skip — blank resume
        </button>
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
        <div style={{ position:"relative", flex:1, minWidth:220 }}>
          <span style={{ position:"absolute", left:11, top:"50%", transform:"translateY(-50%)", fontSize:14, color:"#444460", pointerEvents:"none" }}>🔍</span>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by role, skill, or category…"
            style={{ width:"100%", padding:"10px 13px 10px 36px", background:"#1c1c28", border:"1.5px solid rgba(0,0,0,.1)", borderRadius:10, fontSize:13, color:"#f1f1f5", outline:"none" }}
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
          {TEMPLATE_CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setFilterCat(cat)}
              style={{ padding:"7px 13px", border:`1.5px solid ${filterCat===cat?"var(--teal)":"rgba(255,255,255,0.08)"}`, borderRadius:20, fontSize:12, fontWeight:600, cursor:"pointer", transition:"all .15s", background:filterCat===cat?"rgba(13,148,136,.08)":"rgba(255,255,255,0.04)", color:filterCat===cat?"var(--teal)":"#8888a8" }}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div style={{ fontSize:12, color:"#444460", marginBottom:18 }}>Showing <strong style={{ color:"#374151" }}>{filtered.length}</strong> templates{search ? ` for "${search}"` : ""}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))", gap:14 }}>
        {filtered.map(tpl => (
          <div key={tpl.id}
            ...
            style={{ background:"#1c1c28", border:`2px solid ${hovered===tpl.id?tpl.color:"rgba(255,255,255,0.06)"}`, borderRadius:14, overflow:"hidden", cursor:"pointer", transition:"all .2s", transform:hovered===tpl.id?"translateY(-3px)":"none", boxShadow:hovered===tpl.id?`0 8px 24px ${tpl.color}25`:"0 1px 4px rgba(0,0,0,.05)" }}>
            ...
            <div style={{ position:"relative", borderBottom:`1px solid ${tpl.color}20` }} onClick={() => setPreview(tpl)}>
              ...
              {hovered===tpl.id && (
                <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.4)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <span style={{ color:"white", fontSize:12, fontWeight:700, background:"rgba(0,0,0,0.7)", padding:"4px 10px", borderRadius:6 }}>👁 Preview</span>
                </div>
              )}
            </div>
            ...
            <div style={{ padding:"10px 12px 12px" }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#f1f1f5", marginBottom:2 }}>{tpl.name}</div>
              <div style={{ fontSize:11, color:"#8888a8", marginBottom:8 }}>{tpl.category}</div>
...
function ATSPage() {
  ...
  const analyseResume = React.useCallback(async () => {
    try {
      setAnalysing(true);
  ...
  if (atsMode === "import" && !resume.personal.email && !resume.experience.length) return (
    <div className="fade-up" style={{ maxWidth: 520, margin: "0 auto" }}>
      <button onClick={() => setAtsMode(null)} style={{ background: "none", border: "none", color: "#8888a8", fontSize: 13, cursor: "pointer", marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>← Back</button>
      <h2 style={{ fontFamily: "Bricolage Grotesque, system-ui, sans-serif", fontSize: 26, fontWeight: 800, color: "#f1f1f5", marginBottom: 8, letterSpacing: "-.5px" }}>Import your resume</h2>
      <p style={{ color: "#8888a8", fontSize: 13, marginBottom: 32, lineHeight: 1.7 }}>We'll score your current resume instantly and show exactly what to fix to boost your ATS score.</p>
      ...
      <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, padding: "44px 32px", border: "1px dashed rgba(99,102,241,.35)", borderRadius: 18, cursor: "pointer", background: "rgba(99,102,241,.03)", transition: "all .25s" }}
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12, color: "#8888a8" }}>PDF files only</div>
        </div>
      </label>
      ...
      <button onClick={() => setAtsMode("build")} style={{ marginTop: 10, width: "100%", padding: "12px", background: "transparent", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 13, fontSize: 13, color: "#8888a8", cursor: "pointer" }}>
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
        <div>
          <button onClick={() => setAtsMode(null)} style={{ background: "none", border: "none", color: "#8888a8", fontSize: 13, cursor: "pointer", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>← Resume Maker</button>
          <h2 style={{ fontFamily: "Bricolage Grotesque, system-ui, sans-serif", fontSize: 24, fontWeight: 800, color: "#f1f1f5", letterSpacing: "-.4px" }}>Build your resume</h2>
          {atsResult && <div style={{ fontSize: 11, color: "#8888a8", marginTop: 4 }}>Detected role: <span style={{ color: "var(--teal)" }}>{atsResult.detected_roles?.join(", ")}</span></div>}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => setShowPreview(p => !p)} style={{ padding: "9px 18px", background: showPreview ? "rgba(13,148,136,.2)" : "var(--bg2)", border: `1px solid ${showPreview ? "rgba(13,148,136,.4)" : "rgba(255,255,255,0.06)"}`, borderRadius: 10, color: showPreview ? "var(--teal)" : "var(--text2)", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
        ...
        {!showPreview && (
          <div style={{ width: 240, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 16, padding: "20px 16px", textAlign: "center" }}>
              ...
              {analysing && <div style={{ fontSize: 11, color: "#8888a8", marginTop: 6 }}>Updating score…</div>}
              {atsResult && (
                <div style={{ marginTop: 10, display: "flex", gap: 6, justifyContent: "center" }}>
                  <button onClick={() => setShowBreakdown(b => !b)} style={{ padding: "5px 11px", background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 7, fontSize: 11, color: "#8888a8", cursor: "pointer" }}>
                    {showBreakdown ? "▲ Hide" : "Breakdown"}
                  </button>
                  <button onClick={() => setShowKeywords(b => !b)} style={{ padding: "5px 11px", background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 7, fontSize: 11, color: "#8888a8", cursor: "pointer" }}>
                    Keywords
                  </button>
                </div>
              )}
            ...
            {showBreakdown && atsResult && (
              <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, padding: 16 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#8888a8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 12 }}>Score Breakdown</div>
                {Object.entries(atsResult.breakdown).map(([key, val]) => {
                  ...
                  return (
                    <div key={key} style={{ marginBottom: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 11, color: "#8888a8" }}>{val.label}</span>
            ...
            {showKeywords && atsResult && (
              <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, padding: 16 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#8888a8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 10 }}>Missing Keywords</div>
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: "#8888a8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 10 }}>Found Keywords</div>
            ...
            {/* Suggestions */}
            <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, padding: 16 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#8888a8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 10 }}>
            ...
            <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, overflow: "hidden" }}>
              {SECTIONS.map(s => (
                <button key={s.key} onClick={() => setActiveSection(s.key)} className="nav-btn"
                  ...
                  <span style={{ fontSize: 12, fontWeight: activeSection === s.key ? 700 : 500, color: activeSection === s.key ? "var(--teal)" : "var(--text2)", fontFamily: "Bricolage Grotesque, system-ui, sans-serif" }}>{s.label}</span>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {showPreview ? (
            <div style={{ border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", background: "#16161f", borderBottom: "1px solid var(--sb-border)" }}>
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#8888a8", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 8 }}>Resume Layout</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                    {Object.entries(LAYOUT_META).map(([key, meta]) => (
                      <button key={key} onClick={() => setResumeLayout(key)}
                        style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 11px",
                          background: resumeLayout === key ? resumeColor + "18" : "rgba(255,255,255,0.04)",
                          border: `1.5px solid ${resumeLayout === key ? resumeColor : "rgba(255,255,255,0.08)"}`,
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <span style={{ fontSize: 11, color: "#8888a8" }}>Accent colour:</span>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: "#8888a8", fontFamily: "JetBrains Mono, monospace" }}>resume_preview.pdf</span>
            ...
            <div className="slide-in">
                ...
                <div className="section-card">
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    {[["name","Full Name","e.g. Rahul Sharma"],["email","Email","rahul@email.com"],["phone","Phone","+91 98765 43210"],["location","Location","Bangalore, India"],["linkedin","LinkedIn URL","linkedin.com/in/rahul"],["portfolio","Portfolio / GitHub","github.com/rahul"]].map(([k, label, ph]) => (
                      <div key={k}>
                        <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 5 }}>{label} {["name","email"].includes(k) && <span style={{ color: "#f87171" }}>*</span>}</div>
              ...
              {activeSection === "summary" && (
                <div className="section-card">
                  ...
                  <p style={{ fontSize: 12, color: "#8888a8", marginBottom: 12, lineHeight: 1.6 }}>Write 3–5 sentences: who you are, your top skills, years of experience and what you bring. Mirror keywords from job descriptions you're targeting.</p>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                    <span style={{ fontSize: 11, color: "#8888a8" }}>{resume.summary.split(/\s+/).filter(Boolean).length} words</span>
              ...
              {activeSection === "experience" && (
                <div>
                  {resume.experience.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px", background: "#16161f", border: "1px dashed var(--border2)", borderRadius: 14, marginBottom: 14, color: "#8888a8" }}>
                      <div style={{ fontSize: 32, marginBottom: 10 }}>💼</div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: "#8888a8", marginBottom: 6 }}>No experience added yet</div>
              ...
              {activeSection === "education" && (
                <div>
                  {resume.education.map((edu, i) => (
                    <div key={i} style={{ background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 12, padding: 18, marginBottom: 12 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                        {[["degree","Degree","e.g. B.Tech, B.Sc, MBA"],["field","Field of Study","e.g. Computer Science"],["school","University / College","e.g. IIT Bombay"],["year","Graduation Year","e.g. 2023"],["gpa","GPA / Percentage","e.g. 8.5 / 85%"]].map(([k, label, ph]) => (
                          <div key={k}>
                            <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>{label}</div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {[
                    ...
                    <div key={cat} className="section-card">
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                        <Label>{label}</Label>
                        <span style={{ fontSize: 11, color: "#8888a8" }}>{resume.skills[cat].length} added</span>
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8, minHeight: 28 }}>
                        {resume.skills[cat].length === 0 && <span style={{ fontSize: 12, color: "#8888a8", fontStyle: "italic" }}>None added yet</span>}
                      ...
                      <div style={{ marginTop: 10, fontSize: 11, color: "#8888a8" }}>💡 {tip}</div>
                    </div>
                  ))}
                  ...
                  {atsResult && atsResult.keyword_gaps.length > 0 && (
                    <div style={{ padding: "14px 16px", background: "rgba(239,68,68,.05)", border: "1px solid rgba(239,68,68,.15)", borderRadius: 12 }}>
                      ...
                      <div style={{ fontSize: 11, color: "#8888a8", marginTop: 8 }}>Click a keyword to add it to Technical Skills instantly.</div>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeSection === "certs" && (
                <div className="section-card">
                  ...
                  {resume.certifications.length > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                      {resume.certifications.map((c, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: "#16161f", borderRadius: 10, border: "1.5px solid rgba(0,0,0,.1)" }}>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 600, color: "#f1f1f5" }}>{c.name}</div>
                            <div style={{ fontSize: 11, color: "#8888a8" }}>{c.issuer} {c.year && `· ${c.year}`}</div>
                          </div>
                  ...
                  <div style={{ marginTop: 14, fontSize: 11, color: "#8888a8" }}>💡 Google, AWS, Microsoft, Meta, IBM, and Coursera certs carry the strongest ATS weight. Include completion year.</div>
                </div>
              )}
                ...
                <div>
                  {resume.projects.map((pr, i) => (
                    <div key={i} style={{ background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 12, padding: 18, marginBottom: 12 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
                        <div><div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Project Name</div><input className="dark-input" style={{ width: "100%" }} value={pr.name} onChange={e => updateProject(i, "name", e.target.value)} placeholder="e.g. Customer Churn Predictor" /></div>
                        <div><div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Tech Stack</div><input className="dark-input" style={{ width: "100%" }} value={pr.stack} onChange={e => updateProject(i, "stack", e.target.value)} placeholder="Python, Scikit-learn, Streamlit" /></div>
                        <div><div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>URL / GitHub</div><input className="dark-input" style={{ width: "100%" }} value={pr.url} onChange={e => updateProject(i, "url", e.target.value)} placeholder="github.com/you/project" /></div>
                      </div>
                      <div><div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Description (include outcome + metrics)</div><textarea className="dark-textarea" rows={3} value={pr.description} onChange={e => updateProject(i, "description", e.target.value)} placeholder="Built a machine learning pipeline that predicts customer churn with 89% accuracy using XGBoost, reducing retention costs by 15%…" /></div>
                    </div>
                  ))}
              ...
              {activeSection === "languages" && (
                <div className="section-card">
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Language</div>
                    ...
                    <div style={{ width: 140 }}>
                      <div style={{ fontSize: 11, color: "#8888a8", marginBottom: 4 }}>Proficiency</div>
                      <select value={newLang.level} onChange={e => setNewLang(l => ({ ...l, level: e.target.value }))} style={{ width: "100%", padding: "9px 12px", background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 9, color: "#f1f1f5", fontSize: 13 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20 }}>
                {SECTIONS.findIndex(s => s.key === activeSection) > 0 ? (
                  <button onClick={() => setActiveSection(SECTIONS[SECTIONS.findIndex(s => s.key === activeSection) - 1].key)} style={{ padding: "10px 20px", background: "#16161f", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 10, color: "#8888a8", fontSize: 13, cursor: "pointer" }}>← Previous</button>
                ) : <div />}
                {SECTIONS.findIndex(s => s.key === activeSection) < SECTIONS.length - 1 ? (
                  <button onClick={() => setActiveSection(SECTIONS[SECTIONS.findIndex(s => s.key === activeSection) + 1].key)} style={{ padding: "10px 24px", background: "var(--teal)", border: "none", borderRadius: 10, color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Bricolage Grotesque, system-ui, sans-serif" }}>Next →</button>
                ) : (
                  <button onClick={() => { setShowPreview(true); analyseResume(); }} style={{ padding: "10px 24px", background: "#22c55e", border: "none", borderRadius: 10, color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Bricolage Grotesque, system-ui, sans-serif" }}>Preview & Score ✓</button>
                )}
              </div>
            </div>
          )}
