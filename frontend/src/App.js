        ...
        <main className="page-main">
                ...
                <div className="fade-up" style={{ maxWidth:540, margin:"0 auto" }}>
                  {jobs.loading ? (
                    ...
                    <>
                      ...
                      <label style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:16, padding:"52px 36px", border:`2px dashed ${jobs.file ? C.ocean : "rgba(30,111,212,0.25)"}`, borderRadius:16, cursor:"pointer", background:jobs.file?"rgba(30,111,212,0.04)":"white", transition:"all .2s" }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor=C.ocean; e.currentTarget.style.background="rgba(30,111,212,0.03)"; }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor=jobs.file?C.ocean:"rgba(30,111,212,0.25)"; e.currentTarget.style.background=jobs.file?"rgba(30,111,212,0.04)":"white"; }}>
                        ...
                        <div style={{ textAlign:"center" }}>
                          <div style={{ fontSize:15, fontWeight:600, color:jobs.file?"#1e6fd4":"#64748b", marginBottom:4 }}>{jobs.file ? jobs.file.name : "Click to select your resume"}</div>
                  ...
                  <div className="two-col" style={{ marginBottom:16 }}>
                    <Section label="Job Roles" note="Search anchors — kept constant">
                      <div style={{ display:"flex", flexWrap:"wrap", gap:6, minHeight:28, marginBottom:4 }}>
                        {jobs.roles.length === 0 && <span style={{ fontSize:12, color:"#94a3b8", fontStyle:"italic" }}>No roles detected</span>}
                    ...
                    <Section label="Locations" note="Leave blank for all India">
                      ...
                      <button onClick={h.addLocation} style={{ fontSize:12, color:C.ocean, background:"none", border:"none", cursor:"pointer", marginTop:2, fontWeight:600 }}>+ Add location</button>
                    </Section>
                        ...
                        {EXP_OPTIONS.map(opt => {
                          ...
                          return (
                            <button key={opt.value} onClick={() => h.toggleExp(opt.value)}
                              style={{ padding:"7px 16px", borderRadius:8, border:`1.5px solid ${sel?C.ocean:"rgba(30,111,212,0.15)"}`, fontSize:13, fontWeight:sel?700:500, background:sel?"rgba(30,111,212,0.08)":"white", color:sel?C.ocean:"#64748b", cursor:"pointer", transition:"all .15s" }}>
                              {opt.label}
                            </button>
                          );
                        })}
                      ...
                      {Object.entries(SOURCE_CONFIG).map(([key, cfg]) => {
                        ...
                        return (
                          <button key={key} onClick={() => h.toggleSource(key)}
                            style={{ padding:"8px 18px", borderRadius:20, border:`1.5px solid ${on?cfg.dot:"rgba(30,111,212,0.12)"}`, fontSize:13, fontWeight:on?700:500, background:on?`${cfg.dot}18`:"white", color:on?cfg.color:"#64748b", cursor:"pointer", transition:"all .15s", display:"flex", alignItems:"center", gap:7 }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28, flexWrap:"wrap", gap:12 }}>
                    <div>
                      ...
                      <h2 className="page-title">
                        <span style={{ fontFamily:"'JetBrains Mono',monospace", color:C.ocean, fontWeight:500 }}>{jobs.results.length}</span> Positions Found
                      </h2>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:24 }}>
                    {[["all",`All (${jobs.results.length})`], ...Object.entries(jobs.bySource).filter(([,c])=>c>0).map(([src,count]) => {
                      ...
                      <button key={key} onClick={() => dispatch({ type: A.SET_JOB_FILTER, payload: key })}
                        style={{ padding:"5px 13px", borderRadius:20, fontSize:11.5, fontWeight:700, border:`1.5px solid ${jobs.filterSource===key?C.ocean:"rgba(30,111,212,0.15)"}`, background:jobs.filterSource===key?C.ocean:"white", color:jobs.filterSource===key?"white":"#64748b", cursor:"pointer", transition:"all .15s" }}>
                        {label}
                      </button>
                    ))}
                  </div>
                      ...
                      {h.visibleJobs.map((job, i) => {
                        ...
                        return (
                          <div key={i} className="job-card premium-job-card"
                            style={{ background:"white", border:`1px solid ${C.border}`, borderRadius:18, padding:24, display:"flex", flexDirection:"column", boxShadow:"0 6px 18px rgba(30,111,212,0.08)", animation:`fadeUp .4s ${Math.min(i*0.03,0.35)}s both cubic-bezier(.22,1,.36,1)`, transition:"all .25s ease" }}>
                            ...
                            <h3 style={{ fontFamily:"'Outfit',sans-serif", fontSize:18, fontWeight:800, color:"#0f172a", lineHeight:1.45, marginBottom:14, flex:1, letterSpacing:"-0.3px" }}>{job.title || "Untitled Role"}</h3>
                            ...
                            {job.match && (
                              <div style={{ padding:14, background:`${job.match.bar_color}08`, border:`1px solid ${job.match.bar_color}25`, borderRadius:12, marginBottom:14 }}>
                                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                                  <span style={{ fontSize:10.5, fontWeight:700, color:"#94a3b8", textTransform:"uppercase" }}>Match</span>
                                  <span style={{ fontFamily:"'Outfit',sans-serif", fontSize:20, fontWeight:800, color:job.match.bar_color }}>{job.match.score}%</span>
                                </div>
                                <div style={{ height:5, background:"rgba(30,111,212,0.08)", borderRadius:4, overflow:"hidden" }}>
                            ...
                            <div style={{ display:"flex", gap:8, marginTop:"auto" }}>
                              ...
                              <a href={job.apply_link} target="_blank" rel="noreferrer"
                                style={{ flex:1, display:"block", textAlign:"center", padding:12, background:"linear-gradient(135deg,#1e6fd4,#1558b0)", color:"white", borderRadius:12, fontSize:13.5, fontWeight:700, textDecoration:"none", boxShadow:"0 4px 14px rgba(30,111,212,0.25)" }}>
                                Apply Now →
                              </a>
                            </div>
                          </div>
                    ...
                    <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                      {[["all",`All (${courses.results.length})`], ...Object.entries(courses.byPlatform).map(([p,c])=>[p,`${p} (${c})`])].map(([key,label]) => (
                        <button key={key} onClick={() => dispatch({ type: A.SET_COURSE_FILTER, payload: key })}
                          style={{ padding:"5px 13px", borderRadius:20, fontSize:11.5, fontWeight:700, border:`1.5px solid ${courses.filterPlatform===key?C.ocean:"rgba(30,111,212,0.15)"}`, background:courses.filterPlatform===key?C.ocean:"white", color:courses.filterPlatform===key?"white":"#64748b", cursor:"pointer", transition:"all .15s" }}>
                          {label}
                        </button>
                      ))}
                    </div>
                    <div style={{ display:"flex", gap:6 }}>
                      {[["platform","Platform"],["rating","⭐ Rating"]].map(([k,l]) => (
                        <button key={k} onClick={() => dispatch({ type: A.SET_COURSE_SORT, payload: k })}
                          style={{ padding:"5px 12px", background:courses.sortBy===k?"rgba(30,111,212,0.08)":"white", border:`1.5px solid ${courses.sortBy===k?C.ocean:"rgba(30,111,212,0.15)"}`, borderRadius:8, fontSize:11.5, color:courses.sortBy===k?C.ocean:"#64748b", cursor:"pointer", fontWeight:600 }}>
                          {l}
                        </button>
                      ))}
                    </div>
          ...
          {mode === "alerts" && (
            <div className="fade-up">
              ...
              <div style={{ marginBottom:32 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"1px", marginBottom:14 }}>My Alerts ({alerts.list.length})</div>
                    ...
                    <div key={i} style={{ background:"white", border:`1px solid ${C.border}`, padding:"16px 20px", borderRadius:12, marginBottom:10, boxShadow:"0 1px 3px rgba(30,111,212,0.04)", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                        {alertItem.last_notified_count > 0 && (
                          <div style={{ padding:"4px 12px", background:"rgba(13,148,136,0.08)", border:"1px solid rgba(13,148,136,0.2)", borderRadius:20, fontSize:11.5, fontWeight:700, color:"#0d9488" }}>{alertItem.last_notified_count} new</div>
                        )}
              ...
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"1px" }}>
                    🔔 Notifications {notifs.count > 0 && <span style={{ background:"#e11d48", color:"white", borderRadius:"50%", fontSize:10, padding:"1px 6px", marginLeft:6 }}>{notifs.count}</span>}
                  </div>
                  {notifs.count > 0 && (
                    <button onClick={h.markAllRead} style={{ fontSize:12, color:C.ocean, background:"none", border:"none", cursor:"pointer", fontWeight:700 }}>Mark all read</button>
                  )}
                </div>
                      ...
                      <div style={{ padding:"14px 18px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, cursor:"pointer" }}
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:10, flex:1, minWidth:0 }}>
                          ...
                          <div style={{ minWidth:0 }}>
                            ...
                            <div style={{ fontSize:11.5, color:"#94a3b8", marginTop:2 }}>{new Date(notif.created_at).toLocaleString()} · {notif.jobs?.length || 0} jobs</div>
                          </div>
                        </div>
                        <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                          {notif.jobs?.length > 0 && (
                          ...
                          <span style={{ fontSize:12, color:"#94a3b8" }}>{notifs.expandedId === notif._id ? "▲" : "▼"}</span>
                        </div>
                      </div>
