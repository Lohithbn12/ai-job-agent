      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#020617" }}>
        ...
        <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
          ...
          <nav style={{ flex:1, padding:"12px 10px", display:"flex", flexDirection:"column", gap:2, overflowY:"auto" }}>
            <div style={{ fontSize: 10, color: "#475569", fontWeight: 700, letterSpacing: "1.4px", padding: "4px 10px 10px", textTransform: "uppercase" }}>Navigation</div>
            {navItems.map((item) => (
            ...
            {mode === "jobs" && step > 1 && (
              <div style={{ marginTop: 10, padding: "0 8px" }}>
                <div style={{ width: "100%", height: "1px", background: "#f8fafc", marginBottom: 14 }} />
                <div style={{ fontSize: 10, color: "#475569", fontWeight: 700, letterSpacing: "1px", marginBottom: 10, textTransform: "uppercase" }}>Progress</div>
                {["Upload", "Configure", "Results"].map((s, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, flexShrink: 0, background: step > i + 1 ? "#22c55e" : step === i + 1 ? "var(--teal)" : "rgba(0,0,0,.06)", color: step >= i + 1 ? "white" : "#64748b", boxShadow: step === i + 1 ? "0 0 0 3px rgba(13,148,136,.2)" : "none" }}>{step > i + 1 ? "✓" : i + 1}</div>
                    <span style={{ fontSize: 12, color: step === i + 1 ? "var(--teal)" : step > i + 1 ? "#22c55e" : "#64748b", fontWeight: step === i + 1 ? 600 : 400 }}>{s}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="content-area" style={{ marginLeft: "var(--sb-width)", flex: 1, display: "flex", flexDirection: "column", minWidth: 0, background: "#020617" }}>
          ...
          <header className="top-bar" style={{ height:64, borderBottom:"1px solid var(--border)", display:"flex", alignItems:"center", padding:"0 28px", justifyContent:"space-between", background:"#020b1a", position:"sticky", top:0, zIndex:30, boxShadow:"0 1px 0 rgba(59,130,246,.08), 0 2px 16px rgba(0,0,0,.4)" }}>
            <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              <button className="hamburger" onClick={()=>setSidebarOpen(true)} style={{ display:"none", background:"none", border:"none", color:"#475569", fontSize:18, cursor:"pointer", padding:4, alignItems:"center" }}>☰</button>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              {mode==="jobs"&&step===3&&<span style={{ fontSize:12, color:"#475569", fontWeight:500, padding:"6px 12px", background:"#f8fafc", borderRadius:20 }}>{jobs.length} positions</span>}
            </div>
          </header>
                ...
                {step === 1 && (
                  <div className="fade-up" style={{ maxWidth: 540, margin: "0 auto" }}>
                    <div style={{ marginBottom: 44 }}>
                      <h1 className="hero" style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", fontSize: 42, fontWeight: 800, color: "#0f172a", lineHeight: 1.08, marginBottom: 14, letterSpacing: "-1.2px" }}>
                        Find your<br />
                        <span style={{ background: "var(--teal)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>next role.</span>
                      </h1>
                      <p style={{ color: "#475569", fontSize: 15, lineHeight: 1.75 }}>Upload your resume. We extract roles and skills, then search across 5 platforms simultaneously with experience verification.</p>
                    </div>
                    ...
                    <label style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, padding: "48px 32px", border: "1px dashed var(--border2)", borderRadius: 20, cursor: "pointer", background: "rgba(99,102,241,.02)", transition: "all .25s" }}
                      ...
                      <div style={{ textAlign: "center" }}>
                        ...
                        <div style={{ fontSize: 12, color: "#475569" }}>PDF files only · max 10MB</div>
                      </div>
                    ...
                    <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 28 }}>
                      {[["🎯", "Role-anchored search"], ["🛡", "2-point exp verification"], ["🔀", "5 platforms in parallel"], ["📍", "Multi-location support"]].map(([icon, text], i) => (
                        <div key={i} style={{ padding: "13px 16px", background: "#f8fafc", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 12, fontSize: 12, color: "#475569", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {step === 2 && (
                  <div className="fade-up">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
                      <div>
                        <h2 style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", fontSize: 26, fontWeight: 800, color: "#0f172a", letterSpacing: "-.5px", marginBottom: 5 }}>Configure Search</h2>
                        <p style={{ color: "#475569", fontSize: 13 }}>Edit roles and skills, set locations and experience</p>
                      </div>
                      <button onClick={() => setStep(1)} style={{ padding: "8px 18px", background: "#f8fafc", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 9, color: "#475569", fontSize: 13, cursor: "pointer" }}>← Back</button>
                    </div>
                    ...
                    <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                      <Section label="Job Roles" note="Search anchors — kept constant">
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, minHeight: 28, marginBottom: 4 }}>
                          {roles.length === 0 && <span style={{ fontSize: 12, color: "#475569", fontStyle: "italic" }}>No roles detected</span>}
                      ...
                      <Section label="Top Skills for Search" note={`${topSkills.length} selected · used to build queries`}>
                        {/* Explanation */}
                        <div style={{ fontSize:11.5, color:"#64748b", marginBottom:10, lineHeight:1.6, padding:"8px 10px", background:"rgba(13,148,136,.05)", borderRadius:8, border:"1px solid rgba(13,148,136,.12)" }}>
                        ...
                        <div style={{ display:"flex", flexWrap:"wrap", gap:6, minHeight:28, marginBottom:10 }}>
                          {topSkills.length === 0 && <span style={{ fontSize:12, color:"#94a3b8", fontStyle:"italic" }}>No top skills — add manually below</span>}
                          {topSkills.map((s, i) => (
                        ...
                        {skills.length > 0 && (
                          <div style={{ marginTop:10 }}>
                            <div style={{ fontSize:11, color:"#94a3b8", marginBottom:6, fontWeight:600, textTransform:"uppercase", letterSpacing:".5px" }}>All detected skills — click to add to search:</div>
                            <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                              {skills.filter(s => !topSkills.includes(s)).slice(0, 20).map((s, i) => (
                                <button key={i} onClick={() => setTopSkills(p => [...p, s])}
                                  style={{ padding:"3px 9px", background:"#f1f5f9", border:"1px solid rgba(0,0,0,.1)", borderRadius:5, fontSize:11, color:"#475569", cursor:"pointer", fontWeight:500, transition:"all .12s" }}
                                  ...
                                  onMouseLeave={e => { e.currentTarget.style.borderColor="rgba(0,0,0,.1)"; e.currentTarget.style.color="#475569"; e.currentTarget.style.background="#f1f5f9"; }}>
                                  + {s}
                                </button>
                              ))}
                            </div>
                      ...
                      <Section label="Experience Range" note="Select one or more">
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                          {EXP_OPTIONS.map((opt) => {
                            ...
                            return (
                              <button key={opt.value} onClick={() => toggleExp(opt.value)} style={{ padding: "7px 16px", border: `1px solid ${sel ? "var(--teal)" : "rgba(0,0,0,.06)"}`, borderRadius: 8, fontSize: 13, fontWeight: sel ? 600 : 400, background: sel ? "var(--teal-glow)" : "var(--bg2)", color: sel ? "var(--teal)" : "#64748b", cursor: "pointer", transition: "all .15s" }}>{sel ? "✓ " : ""}{opt.label}</button>
                            );
                          })}
                        </div>
                      </Section>
                    ...
                    <div style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, padding: 20, marginBottom: 20 }}>
                      <Label>Job Sources</Label>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                        {Object.entries(SOURCE_CONFIG).map(([key, cfg]) => {
                          const on = sources.includes(key);
                          return (
                            <button key={key} onClick={() => toggleSource(key)} style={{ padding: "8px 18px", border: `1px solid ${on ? cfg.dot : "rgba(0,0,0,.06)"}`, borderRadius: 22, fontSize: 13, fontWeight: on ? 600 : 400, background: on ? `${cfg.dot}18` : "var(--bg2)", color: on ? cfg.color : "#64748b", cursor: "pointer", transition: "all .15s", display: "flex", alignItems: "center", gap: 7 }}>
                ...
                {step === 3 && (
                  <div className="fade-in">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
                      <div>
                        <h2 className="results-h2" style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", fontSize: 28, fontWeight: 800, color: "#0f172a", letterSpacing: "-.6px", marginBottom: 8 }}>
                          <span style={{ color: "var(--teal)" }}>{jobs.length}</span> Positions Found
                        </h2>
                        {keywordsUsed.length > 0 && (
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 5, alignItems: "center" }}>
                            <span style={{ fontSize: 11, color: "#475569" }}>Searched:</span>
                      ...
                      <button onClick={() => setStep(2)} style={{ padding: "8px 18px", background: "#f8fafc", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 9, color: "#475569", fontSize: 13, cursor: "pointer" }}>← Refine</button>
                    </div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 24 }}>
                      ...
                      {Object.entries(bySource).filter(([, c]) => c > 0).map(([src, count]) => {
                        const cfg = SOURCE_CONFIG[src] || { label: src, color: "#475569", dot: "var(--text2)" };
                    ...
                    {visibleJobs.length === 0 ? (
                      <div style={{ textAlign: "center", padding: "80px 0" }}>
                        ...
                        <div style={{ fontSize: 16, fontWeight: 600, color: "#475569", marginBottom: 6, fontFamily: "Plus Jakarta Sans, system-ui, sans-serif" }}>No results</div>
                        <div style={{ fontSize: 13, color: "#475569" }}>Try a broader experience range or different keywords</div>
                      </div>
                    ) : (
                      <div className="card-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(310px,1fr))", gap: 14 }}>
                        {visibleJobs.map((job, i) => {
                          ...
                          const sc = SOURCE_CONFIG[sk] || { label: job.source, color: "#475569", dot: "var(--text2)" };
                          return (
                            <div key={i} className="job-card" style={{ background: "#ffffff", border: `1px solid ${job.exp_hard_mismatch ? "rgba(220,38,38,.2)" : "rgba(0,0,0,.06)"}`, borderRadius: 16, padding: 22, display: "flex", flexDirection: "column", animation: `fadeUp .45s ${Math.min(i * .035, .4)}s both cubic-bezier(.22,1,.36,1)` }}>
                              ...
                              <h3 style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", lineHeight: 1.45, marginBottom: 12, fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", flex: 1 }}>{job.title || "Untitled Role"}</h3>
                              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                                  ...
                                  {job.exp_required && <span style={{ fontSize: 11, color: "#475569" }}>· requires <strong style={{ color: "#475569" }}>{job.exp_required}</strong></span>}
                                </div>
                              </div>
                              ...
                              {job.match && (
                                <div style={{ marginTop:12, padding:"11px 13px", background:`${job.match.bar_color}0d`, border:`1px solid ${job.match.bar_color}30`, borderRadius:10 }}>
                                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:7 }}>
                                    <span style={{ fontSize:11, fontWeight:700, color:"#475569" }}>🎯 Application Match</span>
                                  ...
                                  {job.match.tip && <div style={{ fontSize:10.5, color:"#64748b", lineHeight:1.5 }}>{job.match.tip}</div>}
                                </div>
                              )}
            ...
            {mode === "courses" && (
              <div className="fade-up">
                <div style={{ marginBottom: 36 }}>
                  <h1 className="hero" style={{ fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", fontSize: 36, fontWeight: 800, color: "#0f172a", lineHeight: 1.1, letterSpacing: "-1px", marginBottom: 10 }}>
                    Free <span style={{ background: "var(--teal)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Courses</span>
                  </h1>
                  <p style={{ color: "#475569", fontSize: 14, lineHeight: 1.7 }}>Search YouTube, Coursera, edX, Simplilearn and Google — only free resources.</p>
                </div>

                <div style={{ display: "flex", gap: 0, background: "#f8fafc", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 14, overflow: "hidden", marginBottom: 32 }}>
                  <div style={{ display: "flex", alignItems: "center", paddingLeft: 16, color: "#475569", fontSize: 16 }}>🔍</div>
                  <input value={courseKeyword} onChange={e => setCourseKeyword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleCourseSearch()} placeholder="e.g. Data Analyst, Python, Machine Learning…" style={{ flex: 1, padding: "14px 14px", background: "none", border: "none", fontSize: 14, color: "#0f172a" }} />
                  ...
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        ...
                        {Object.entries(byPlatform).map(([p, c]) => {
                          const cfg = PLATFORM_CONFIG[p] || { color: "#94a3b8" };
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        {[["platform", "Platform"], ["rating", "⭐ Rating"]].map(([k, l]) => (
                          <button key={k} onClick={() => setSortCourses(k)} style={{ padding: "6px 12px", background: sortCourses === k ? "rgba(34,211,238,.12)" : "var(--bg2)", border: `1px solid ${sortCourses === k ? "rgba(34,211,238,.35)" : "rgba(0,0,0,.06)"}`, borderRadius: 8, fontSize: 12, color: sortCourses === k ? "#22d3ee" : "#64748b", cursor: "pointer" }}>{l}</button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="card-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 14 }}>
                      {filteredCourses.map((course, i) => {
                        const pCfg = PLATFORM_CONFIG[course.platform] || { color: "#475569", icon: "○" };
                        return (
                          <div key={i} className="job-card" style={{ background: "#ffffff", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 16, padding: 22, display: "flex", flexDirection: "column", animation: `fadeUp .45s ${Math.min(i * .035, .4)}s both cubic-bezier(.22,1,.36,1)` }}>
                            ...
                            <h3 style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", lineHeight: 1.5, marginBottom: 12, fontFamily: "Plus Jakarta Sans, system-ui, sans-serif", flex: 1 }}>{course.title || "Untitled Course"}</h3>
                ...
                {!courseStatus && (
                  <div>
                    <p style={{ fontSize: 12, color: "#475569", marginBottom: 14, textTransform: "uppercase", letterSpacing: "1px", fontWeight: 600 }}>Platforms covered</p>
                    <div className="card-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 10 }}>
                      {Object.entries(PLATFORM_CONFIG).map(([name, cfg]) => (
                        <div key={name} style={{ padding: "20px 12px", background: "#f8fafc", border: "1.5px solid rgba(0,0,0,.1)", borderRadius: 12, textAlign: "center" }}>
                ...
                {!courseLoading && courses.length === 0 && courseStatus && (
                  <div style={{ textAlign: "center", padding: "70px 0" }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 600, color: "#475569", fontFamily: "Plus Jakarta Sans, system-ui, sans-serif" }}>No courses found</div>
                    <div style={{ fontSize: 13, color: "#475569", marginTop: 6 }}>Try a different keyword</div>
                  </div>
                )}
              </div>
            )}
